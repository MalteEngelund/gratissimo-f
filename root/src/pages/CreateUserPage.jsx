...
export function CreateUserPage() {
  ...
  const createLogin = async (data) => {
    try {
      const response = await fetch(import.meta.env.VITE_PUBLIC_BASE_URL + '/api/users/', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        console.log('Login created successfully:', responseData)
        setLoginResponse('Konto oprettet!')
      }
      if (!response.ok) {
        console.error('Failed to create login:', responseData)
        setLoginResponse('Fejl ved oprettelse af konto. Prøv igen.')
      }
  ...
  return (
    <>
      <SectionContainer>
        <SectionTitle text='Opret bruger' />
        {!authToken ? (
        <form noValidate onSubmit={handleSubmit(createLogin)} className='w-full flex flex-col gap-8'>
          ...
          <Button type='submit' text='Opret bruger' />
